    ...
    <div v-else>
      ...
      <div class="border-4 border-white bg-gray-50 shadow-xl rounded-lg">
        <table class="text-left w-full table-container">
          ...
          <tr v-for="el in simulatedStocks" :key="el._id" class="bg-gray-100 text-center text-black font-bold border-b-2 border-gray-300">
...
Cost Basis -> how much you paid (shares * stock price(at the time you bought stock))
Market Price -> shares * current stock price
P/L -> Market Price - Cost Basis (I think)
...
export default {
  ...
  methods: {
    ...
    async getMarketPrice(ticker) {
      ...
      const test = await this.$store.dispatch('watchlist/fetchStockPrices', ticker)
